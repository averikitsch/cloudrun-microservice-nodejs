...
const {fetchIdToken} = require('../metadata');
...
describe('System Tests', () => {
  ...
  if (!SERVICE_NAME) {
    SERVICE_NAME = 'cloudrun-template-app';
    console.log(
  ...
  before(async () => {
    ...
    let buildCmd =
      ...
      '--config ./test/setup.cloudbuild.yaml ' +
    ...
    ID_TOKEN = await fetchIdToken(BASE_URL);
  ...
  after(() => {
    let cleanUpCmd =
      `gcloud builds submit --project ${GOOGLE_CLOUD_PROJECT} ` +
      '--config ./test/cleanup.cloudbuild.yaml ' +
      `--substitutions _SERVICE=${SERVICE_NAME},_PLATFORM=${PLATFORM},_REGION=${REGION}`;
